...
/** Stored profile shape: { teacher_profile?: { identity?, teaching_style?, goals?, ... } } */
type StoredProfile = Record<string, unknown> & {
  teacher_profile?: Record<string, unknown> & {
    identity?: Record<string, unknown> & { name?: string; age?: number; years_experience?: number; subjects_taught?: string[]; grade_levels?: string[]; certifications?: string[] };
    teaching_style?: Record<string, unknown> & { primary_approach?: string; secondary_approach?: string };
    goals?: Record<string, unknown> & { primary_goal?: string; secondary_goal?: string; professional_development_focus?: string };
...
function displayMessage(text: string): string {
  let out = text
    .replace(/TEACHER_PROFILE_COMPLETE\s*/i, "")
    .trim();
  const jsonStart = out.search(/\s*\{[\s\S]*"teacher_profile"/);
  if (jsonStart >= 0) {
    out = out.slice(0, jsonStart).trim();
  }
  return out || "Your profile is ready.";
}
...
function ProfileView({ profile }: { profile: StoredProfile }) {
  const tp = profile.teacher_profile;
  const identity = tp?.identity;
  const teachingStyle = tp?.teaching_style;
  const goals = tp?.goals;

  const section = (title: string, children: React.ReactNode) => (
    <div className="border border-[#d6d6d6] border-solid flex flex-col gap-[8px] p-[16px] w-full">
      <p className="font-bold text-[12px] text-[rgba(0,0,0,0.7)] uppercase">
        {title}
      </p>
      {children}
    </div>
  ...
  const row = (label: string, value: string | number | string[] | undefined) => {
    if (value == null) return null;
    const text = Array.isArray(value) ? value.join(", ") : String(value);
    if (!text) return null;
    return (
      <p key={label} className="font-normal text-[14px] text-black">
        <span className="text-[rgba(0,0,0,0.7)]">{label}: </span>
        {text}
      </p>
    );
  };
  ...
  return (
    <div className="flex flex-col gap-[16px] w-full max-w-[720px] items-center">
      <div className="flex flex-wrap items-center justify-center gap-4 w-full">
        <h2 className="font-bold text-[24px] text-[#011627] text-center">
          {identity?.name ? `${identity.name}'s profile` : "Teacher profile"}
        </h2>
      </div>
      ...
      {identity && section(
        ...
        <>
          ...
          {row("Years of experience", identity.years_experience)}
          {row("Subjects taught", identity.subjects_taught)}
          {row("Grade levels", identity.grade_levels)}
          {row("Certifications", identity.certifications)}
        </>
      )}

      {teachingStyle && (teachingStyle.primary_approach || teachingStyle.secondary_approach) && section(
        "Teaching style",
        <>
          {row("Primary approach", teachingStyle.primary_approach)}
          {row("Secondary approach", teachingStyle.secondary_approach)}
        </>
      )}

      {goals && (goals.primary_goal || goals.secondary_goal || goals.professional_development_focus) && section(
        "Goals",
        <>
          {row("Primary goal", goals.primary_goal)}
          {row("Secondary goal", goals.secondary_goal)}
          {row("Professional development", goals.professional_development_focus)}
        </>
      )}

      <Link
        href="/"
        className="bg-[#05c770] text-white font-bold text-[14px] uppercase px-[16px] py-[12px] w-fit"
      >
        Back to Home
      </Link>
    </div>
...
export default function ProfileChatPage() {
  ...
  const sendTurn = useCallback(
    async (nextMessages: Message[]) => {
      ...
      try {
        ...
        const { assistantMessage, profile: p } = data as {
          assistantMessage: string;
          profile: object | null;
        };
        // Use the messages we sent so we don't lose any due to state timing
  ...
  const handleSend = useCallback(() => {
    ...
    setInput("");
    sendTurn(next);
  }, [input, loading, profile, messages, sendTurn]);
      ...
      <div className="bg-white flex flex-col gap-[12px] items-center pb-[80px] relative min-h-screen w-full">
            ...
            <div className="flex flex-row items-center max-w-[inherit] size-full">
              <div className="content-stretch flex items-center justify-between max-w-[inherit] px-[32px] relative w-full flex-wrap gap-4">
                ...
                <div className="content-stretch flex gap-[24px] items-center justify-end relative shrink-0">
                  ...
                  <Link href="/profile-chat" className="bg-[#05c770] content-stretch flex flex-col items-center justify-center px-[16px] py-[12px] relative shrink-0">
                    <p className="font-bold leading-[1.2] relative shrink-0 text-[16px] text-white uppercase">Profile</p>
                  </Link>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-[1280px] w-full shrink-0">
          <div className="flex items-center justify-center px-[32px] relative w-full">
            <p className="font-normal relative shrink-0 text-[12px] text-[rgba(0,0,0,0.7)] uppercase">Profile</p>
          </div>
        </div>
  ...
  return (
    <div className="bg-white flex flex-col gap-[12px] items-center pb-[80px] relative min-h-screen w-full">
      {/* Onboarding: centered Slate logo only (no nav) */}
      <div className="flex items-center justify-center w-full py-[32px] shrink-0">
        <NavLogo className="h-14 w-auto sm:h-16" />
      </div>

      {/* Breadcrumb */}
      <div className="max-w-[1280px] w-full shrink-0">
        <div className="flex items-start max-w-[inherit] px-[32px] relative w-full">
      ...
      {/* Main chat area */}
      <div className="max-w-[1280px] w-full flex-1 flex flex-col min-h-0 px-[32px]">
        <div className="bg-white relative w-full flex-1 flex flex-col border border-[#d6d6d6] border-solid min-h-[400px]">
          <div className="flex flex-col gap-[16px] items-stretch p-[16px] flex-1 overflow-y-auto min-h-0">
            {messages.length === 0 && !loading && (
              <div className="bg-[rgba(5,199,112,0.08)] border border-[#05c770] border-solid flex flex-col gap-[12px] items-center justify-center p-[24px] text-center">
                <p className="font-bold text-[18px] text-[#011627]">
                  Build your teacher profile
                </p>
                <p className="font-normal text-[14px] text-[rgba(0,0,0,0.7)] max-w-[400px]">
                  Chat with the assistant to create your professional profile. Answer a few questions about your teaching role, experience, and goals.
                </p>
                <button
                  type="button"
                  onClick={handleStart}
                  className="bg-[#05c770] text-white font-bold text-[16px] uppercase px-[16px] py-[12px] hover:opacity-90 transition-opacity"
                >
                  Start conversation
                </button>
              </div>
            ...
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] rounded-none px-[12px] py-[10px] ${
                    m.role === "user"
                      ? "bg-[#05c770] text-white"
                      : "bg-[#f3f3f5] text-black border border-[#d6d6d6] border-solid"
                  }`}
                >
                  <p className="font-medium text-[14px] leading-[1.5] whitespace-pre-wrap">
                    {m.role === "assistant" ? displayMessage(m.content) : m.content}
                  </p>
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-[#f3f3f5] text-black border border-[#d6d6d6] border-solid px-[12px] py-[10px]">
            ...
            {error && (
              <div className="bg-[#f13704]/10 border border-[#f13704] border-solid px-[12px] py-[10px]">
                <p className="font-medium text-[14px] text-[#f13704]">{error}</p>
              </div>
            )}

            {profile && (
              <div className="bg-[rgba(5,199,112,0.12)] border border-[#05c770] border-solid flex flex-col gap-[8px] p-[16px]">
                <p className="font-bold text-[16px] text-[#011627]">
                  Profile complete
                </p>
                <p className="font-normal text-[14px] text-[rgba(0,0,0,0.8)]">
                  Your teacher profile has been created. You can use it to get personalized challenges and recommendations.
                </p>
                <Link
                  href="/"
                  className="bg-[#05c770] text-white font-bold text-[14px] uppercase px-[12px] py-[8px] w-fit"
                >
                  Back to Home
                </Link>
              </div>
          ...
          {messages.length > 0 && !profile && (
            <div className="border-t border-[#d6d6d6] border-solid p-[16px] flex gap-[12px] items-end">
              <textarea
                value={input}
                ...
                rows={2}
                className="flex-1 min-w-0 resize-none border border-[#d6d6d6] border-solid p-[12px] text-[14px] font-normal text-black placeholder:text-[rgba(0,0,0,0.5)] focus:outline-none focus:border-[#05c770]"
                disabled={loading}
              />
              <button
                type="button"
                ...
                className="bg-[#05c770] text-white font-bold text-[14px] uppercase px-[16px] py-[12px] shrink-0 disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
              >
                Send
              </button>
            </div>
